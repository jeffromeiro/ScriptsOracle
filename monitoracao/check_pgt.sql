spool /home/oracle/chklist/log/check_pgt_&1
...
begin
   ...
   into v_num_last_ptt
   from table(cast(a149072.f_long('PROD_DBA', 'PGT') as t_long)) t;
  ...
  -- retorna data correspondente ao numero encontrado na query anterior
   ...
   if  to_date(v_dt_last_ptt,'MON-YYYY') >= to_date(v_atual,'MON-YYYY') then
      ...
      dbms_output.put_line('PGT_' || v_base || '_OK');

   else
      dbms_output.put_line('PGT_' || v_base || '_NOK');
