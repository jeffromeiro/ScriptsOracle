...
accept p_btime       char   prompt 'Inicio (DDMMYYYYHH24MI): '
accept p_etime       char   prompt '   Fim (DDMMYYYYHH24MI): '

set echo off 
WITH ash_query AS (
     SELECT inst_id,substr(event,6,2) lock_type,program, 
     ...
     WHERE event LIKE 'enq: %' and sample_time between to_date('&p_btime','DDMMYYYYHH24MI') 
   AND  to_date('&p_etime','DDMMYYYYHH24MI')
     GROUP BY inst_id,substr(event,6,2) ,program, h.module, h.action, 
