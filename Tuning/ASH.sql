...
QUERY 1.
	...
	REPHEADER LEFT COL 9 '***************************************************************' SKIP 1 -
				   COL 9 '* TOP 10 EVENTOS DE ESPERA NA ULTIMA MEIA HORA, POR INSTANCIA *' SKIP 1 -
				   COL 9 '***************************************************************' SKIP 2
	COL inst_id FOR 99
	COL event   FOR A60
...
SELECT inst_id, sql_id, event, COUNT(1) 
  FROM gv$active_session_history
 WHERE sql_id = '&sql_id' 
   AND sample_time /* ULTIMA MEIA-HORA */ BETWEEN SYSDATE - 60/2880 AND SYSDATE
 GROUP BY inst_id
,         sql_id
,         event 
 ORDER BY 1, 4 DESC
