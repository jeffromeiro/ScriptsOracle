...
BEGIN
  ...
  FOR VAL_SQLID IN (SELECT a.sql_id, a.sql_text
   ...
   LOOP
    ...
    -- Busca as binds da última execução
	FOR VAL_BINDS IN
	(
    ...
    TAMANHO_LOB  := lengthB(VAL_SQLID.sql_text);
    DIVISOR := CEIL(TAMANHO_LOB / 30000); -- divide o lob por 30000 devido a limitação do varchar2 e arredonda o resultado para cima
   
  
    FOR PARTE_DO_TEXTO IN 1 .. DIVISOR LOOP
	  
	  FORMATA_TEXTO := instr(dbms_lob.substr(VAL_SQLID.SQL_TEXT, 30000, CONTADOR),',',-1); -- RETORNA A QUANTIDADE DE CARACTERES ATÉ A ULTIMA VIRGULA
